...
The physical commands used for this historical report are now quarantined
because they bypassed the authenticated unified bridge. The supported
reproduction below is simulation-only; the archived physical artifacts cannot
be regenerated by invoking the legacy sys-ID or calibrated E2E scripts.
...
The gate exits 0 only when all four sim-side checks pass. A physical rerun
requires a purpose-built authenticated command-envelope client; do not use the
historical direct-backend scripts to command hardware.
