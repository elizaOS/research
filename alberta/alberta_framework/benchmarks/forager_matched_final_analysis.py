...
Creation fully replays the exact seal and completed 6-by-30 evaluation around the two resolvers.
The open subject is resolved first; the evaluation resolver is the last external callback, after
which an internal content replay and source/runtime capture precede deterministic statistics and
atomic publication.  Persisted bindings are cache content only: loading this bundle never
authenticates them and never confers promotion, SOTA, or unrestricted performance-claim authority.

The published tree is self-contained for scalar-statistics and digest-closure replay: it contains
an exact seal subtree, an exact snapshot of every immutable and final evaluation artifact, and
the statistics contract/result.  It intentionally does not duplicate raw cell archives, reward
traces, qualification source archives, or executable source bytes; recomputing workloads or
scores still requires the original qualified campaign roots.  Its interpretation is limited to
the selected six-arm execution panel and the frozen named primary contrast among its four selected
inferential arms.  Fixed descriptive arms never enter inferential tests, and the secondary
sign-flip/Holm output is nonconfirmatory sensitivity analysis because the frozen protocol does not
state sign exchangeability.
"""
...
def _analysis_source_records() -> dict[str, dict[str, Any]]:
    ...
    if len(set(roles)) != len(roles) or len(set(paths)) != len(paths):
        raise ForagerMatchedFinalAnalysisError(
            "analysis source closure repeats a role or resolved path"
        )
...
def _analysis_runtime_source_identity() -> dict[str, Any]:
    ...
    body: dict[str, Any] = {
        ...
        "classification": "exact_local_source_closure_and_versioned_runtime_identity",
...
def _parse_analysis_runtime_source(raw: bytes) -> Mapping[str, Any]:
    ...
    if payload != expected:
        raise ForagerMatchedFinalAnalysisError(
            "analysis runtime/source identity differs from executing code or runtime"
        )
...
def _parse_bindings(
    ...
    *,
    ...
    result = evidence.AuthenticatedEvidenceBindings(
        ...
        protocol_sha256=_require_sha256(value["protocol_sha256"], "bindings protocol"),
...
def _validate_qualification_manifest(
    ...
    for name in (
        ...
        if type(payload.get(name)) is not dict:
            raise ForagerMatchedFinalAnalysisError(
                f"evaluation qualification manifest {name} must be an object"
            )
    ...
    return campaign._canonical_sha256(payload)
...
def _validate_evaluation_snapshot(
    ...
    qualification_manifest_sha256 = _validate_qualification_manifest(
    ...
    if type(image_inspection) is not dict:
        raise ForagerMatchedFinalAnalysisError(
            "evaluation live-runtime image inspection must be an object"
        )
    ...
    evaluation.parse_sealed_evaluation_schedule(
        ...
        expected_schedule_sha256=cast(str, schedule_payload["schedule_sha256"]),
    )
    if (
    ...
    expected_completion = {
        ...
        "executor_manifest_sha256": scores.executor_evidence_sha256,
        "live_runtime_identity_sha256": live_runtime_sha256,
        "execution_schedule_sha256": schedule_payload["schedule_sha256"],
        "seal_manifest_payload_sha256": seal_content.manifest["payload_sha256"],
        "open_verification_subject_sha256": (
            seal_content.open_verification_request.verification_subject_sha256
        ),
    ...
    expected_campaign = _expected_campaign_manifest(
        ...
        qualification_manifest_sha256=qualification_manifest_sha256,
        live_runtime_sha256=live_runtime_sha256,
        schedule_sha256=cast(str, schedule_payload["schedule_sha256"]),
    )
    if campaign_manifest != expected_campaign:
        raise ForagerMatchedFinalAnalysisError(
...
def _build_manifest(
    *,
    ...
    if (
        ...
        raise ForagerMatchedFinalAnalysisError(
            "analysis claim boundary is not the exact selected-six partition"
        )
    ...
    body: dict[str, Any] = {
        ...
        "authority_boundary": {
            ...
            "fresh_external_resolver_revalidation_required": True,
            "self_authentication_forbidden": True,
        },
        "claim_boundary": {
            "scope": "selected_six_panel_only",
            "selected_six_candidate_ids": list(candidate_order),
        ...
        "self_contained_scope": {
            "scope": "scalar_statistics_and_digest_closure",
            ...
            "analysis_replay_requires_matching_local_source_and_runtime": True,
        ...
        "statistics": {
            ...
            "primary_inferential_candidate_ids": [
...
def _load_from_open_root(
    ...
    try:
        ...
        if analysis_artifacts["statistics-contract.json"] != _canonical(
            ...
            raise ForagerMatchedFinalAnalysisError(
                "persisted statistics contract differs from exact source replay"
            )
        ...
        if replayed_result != result:
            raise ForagerMatchedFinalAnalysisError(
                "persisted statistics result differs from exact source replay"
            )
        if seal._plain(analysis_runtime_source) != _analysis_runtime_source_identity():
            raise ForagerMatchedFinalAnalysisError(
...
def load_final_analysis_content(
    ...
    """Strictly replay a self-contained bundle without invoking either resolver."""
...
def _cleanup_owned_staging(
    ...
    try:
        ...
        for child_name, artifacts in children:
            child = _open_child(staging, child_name, "owned cleanup subtree")
            try:
                _directory_inventory(
                    child,
                    expected_files=artifacts,
                    label="owned cleanup subtree",
                )
                for entry_name in _pair_names(artifacts):
                    os.unlink(entry_name, dir_fd=child.descriptor)
            finally:
                os.close(child.descriptor)
            os.rmdir(child_name, dir_fd=staging.descriptor)
        for entry_name in _pair_names(_ROOT_FILES):
...
def create_forager_matched_final_analysis_bundle(
    ...
    *,
    ...
    """Replay, freshly authenticate, analyze, and atomically publish final content."""
    if not all(
    ...
    if _analysis_runtime_source_identity() != analysis_runtime_source:
        raise ForagerMatchedFinalAnalysisError(
