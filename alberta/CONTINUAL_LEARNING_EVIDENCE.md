...
| No catastrophic forgetting (optimizer-level) | UPGD retains task-1 accuracy vs σ=0 twin, 24 paired seeds, savings on revisit | mechanism | `test_forgetting_gate.py` |
...
| Multi-agent: endogenous non-stationarity | learning-opponent stream (drift = another learner's learning curve; IDBD's α tracks the opponent's learning phase); adversarial pursuit drives a frozen predictor to 1771 MSE (48x growth) while continual learners hold ≤0.12 | integrated | `test_multiagent_sim.py` |
| Multi-agent: convention memory | two learning agents recall joint conventions instantly on rule recurrence with context (t=20 floor, reward 0.94) vs relearning forever without (t≈50, 0.78); conventions are emergent (≥3 distinct across seeds) | integrated | `test_multiagent_sim.py` |
...
run + 144 s compile for the loop-unrolled path; measured 65,536 demons at
4.0e7 demon-updates/s on CPU — `tests/test_stacked_horde.py` asserts exact
semantics, analytic fixed points, all-1024-demons-learn, and generous time
bounds) ·
...
2. **Compositional next steps** (research program, not gaps in the
   ...
   *hidden* context; richer regime models beyond per-(state, action) reward
   tables; and a learned rather than scripted partner. Seeds 46–59 were not
